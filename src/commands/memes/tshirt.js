module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(vars.gifFormats.find(f => f === type.ext))) {
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/image/tshirt.png -i assets/image/tshirtmask.png -i assets/image/white.png -filter_complex "${squareS.constraint !== 'both' ? `[0:v]crop=x=${squareS.constraint === 'height' ? 0 : 'iw/2-ih/2'}:y=${squareS.constraint === 'width' ? 0 : 'ih/2-iw/2'}:w=${squareS.constraint === 'width' ? 'ih' : 'iw'}:h=${squareS.constraint === 'height' ? 'iw' : 'ih'}[cframe];` : ''}[${squareS.constraint !== 'both' ? 'cframe' : '0:v'}][1:v]scale=rw:rh[image];[3:v][1:v]scale=w=rw*1.25:h=rh+(ow-rw)[white];[image][2:v]overlay=x=0:y=0:format=auto[imagemask];[imagemask][1:v]overlay=x=0:y=0:format=auto[imagetshirt];[white][imagetshirt]overlay=x=(W-w)/2:y=(H-h)/2:format=auto[out]" -map "[out]" -preset ${findpreset(args)} ${filepath}/output.png`)
