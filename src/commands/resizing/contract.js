module.exports = {
    ...
    execute: async function (msg, args) {
        ...
        if (type.mime.startsWith('image') && !(vars.gifFormats.find(f => f === type.ext))) {
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/image/transparent.png -filter_complex "[1:v][0:v]scale2ref=round(iw/${multiplier}):round(ih/${multiplier})[background][input];[background][input]overlay=x=${originx}:y=${originy}:format=auto[out]" -map "[out]" -preset ${findpreset(args)} -aspect ${fileinfo.info.width}:${fileinfo.info.height} ${filepath}/output.png`)
            return await sendFile(msg, filepath, `output.png`)
        } else if (type.mime.startsWith('video')) {
            var filepath = await downloadFile(currenturl, `input.mp4`, {
                fileinfo            })
            var filename = `input.mp4`
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/image/transparent.png -map 0:a? -filter_complex "[1:v][0:v]scale2ref=round(iw/${multiplier}):round(ih/${multiplier})[background][input];[background][input]overlay=x=${originx}:y=${originy}:format=auto,scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${findpreset(args)} -aspect ${fileinfo.info.width}:${fileinfo.info.height} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
            return await sendFile(msg, filepath, `output.mp4`)
        } else if (type.mime.startsWith('image') && vars.gifFormats.find(f => f === type.ext)) {
            var filepath = await downloadFile(currenturl, `input.gif`, {
                fileinfo            })
            var filename = `input.gif`
            await execPromise(`ffmpeg -i ${filepath}/${filename} -i assets/image/transparent.png -filter_complex "[1:v][0:v]scale2ref=round(iw/${multiplier}):round(ih/${multiplier})[background][input];[background][input]overlay=x=${originx}:y=${originy}:format=auto,split[pout][ppout];[ppout]palettegen=reserve_transparent=1[palette];[pout][palette]paletteuse=alpha_threshold=128[out]" -map "[out]" -preset ${findpreset(args)} -aspect ${fileinfo.info.width}:${fileinfo.info.height} -gifflags -offsetting ${filepath}/output.gif`)
    ...
    help: {
        name: 'contract [multiplier (from 1 to 6)] {file} [-origin <x (left/center/right)> <y (top/middle/bottom)>]',
        value: 'Contracts the width and height of the file.'
    },
    cooldown: 2500,
    type: 'Resizing'
}