    ...
    execute: async function (msg, args) {
        ...
        if (filterindex > -1) {
            if (filters.find(filter => filter === args[filterindex + 1].toLowerCase())) {
                ...
                await msg.reply('Not a supported filter.').catch(() => { })
                return
            }
        }
        ...
        if (type.mime.startsWith('image') && !(vars.gifFormats.find(f => f === type.ext))) {
            ...
            await execPromise(`ffmpeg -i ${filepath}/${filename} -filter_complex "[0:v]alphaextract,${filter}=${multiplier}[mask];[0:v]${filter}=${multiplier}[input];[input][mask]alphamerge[out]" -map "[out]" -preset ${findpreset(args)} ${filepath}/output.png`)
            return await sendFile(msg, filepath, `output.png`)
        } else if (type.mime.startsWith('video')) {
            var filepath = await downloadFile(currenturl, `input.mp4`, {
                fileinfo            })
            var filename = `input.mp4`
            await execPromise(`ffmpeg -i ${filepath}/${filename} -map 0:a? -filter_complex "[0:v]${filter}=${multiplier},scale=ceil(iw/2)*2:ceil(ih/2)*2[out]" -map "[out]" -preset ${findpreset(args)} -c:v libx264 -pix_fmt yuv420p ${filepath}/output.mp4`)
